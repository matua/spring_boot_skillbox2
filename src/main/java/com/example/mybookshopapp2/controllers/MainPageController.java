...
public class MainPageController {
    ...
    public BooksPageDto getBooksPage(@RequestParam("offset") Integer offset,
                                     ...
                                     @RequestParam("to") String to, HttpServletRequest request) {
        String path = request.getServletPath();
        if (path.endsWith("recommended")) {
            return new BooksPageDto(bookService.getPageOfRecommendedBooks(offset, limit).getContent());
        } else if ((path.endsWith("recent"))) {
            return new BooksPageDto(bookService.getPageOfBooksFilteredByDate(0, 6, Date.valueOf(JsToDataSqlConverter.convert(from)), Date.valueOf(JsToDataSqlConverter.convert(to))).getContent());
        } else {
